...
@RefreshScope		// Los @Components son singleton, con esta anotacion hacemos que se refresque la instancia al invocar una URL dada 
@RestController
...
//	@Qualifier("itemServiceImpl")
	...
	public ResponseEntity<?> obtenerConfig(@Value("${server.port}") String port) {
		...
		if (env.getActiveProfiles().length > 0 && env.getActiveProfiles()[0].equals("dev")) { // el indice [0] es el nombre del profile activo 
